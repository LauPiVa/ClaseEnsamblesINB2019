%Tarea 2 Ana Laura Pinedo Vargas
%9 Canales
%El canal 1 es el tiempo
clear 
load('15_Ene_2014_2condiciones_filtradoPasaBaja350Hz.mat', 'jCanales')
figure(1), clf

Fs=25e3; %frecuencia de muestreo
tiempoGrafica=600; %segundos que quiero plotear
umbrales = [3e-4 3e-4 2e-4 2e-4 0.5e-3 2e-4 5.5e-4 7.5e-4 6.5e-4];
tiempo_ms=jCanales(1,1:Fs*tiempoGrafica); %tiempo del canal 1
for i=2:10
    correlacion=[]; % Reseteas la matriz de correlacion
    figure(1), clf
    trazo=-jCanales(i,1:Fs*tiempoGrafica); % trazo del canal
